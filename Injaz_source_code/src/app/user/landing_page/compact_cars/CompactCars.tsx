...
interface CompactCars {
  data: any;
  phoneData: any;
}

const CompactCars: React.FC<CompactCars> = ({ data, phoneData }) => {
  const [Rows, setrows] = useState([]);
  const [loader, setLoader] = useState(true);
  const router = useRouter();
  useEffect(() => {
    if (data.length > 0) {
      const newData = data?.filter((item: any) => item.category === "COMPACT");
      setrows(newData);
      setLoader(false);
    }
  }, [data]);
    ...
    <div className="car_offers">
      <Container maxWidth="xl">
        <div className="car_off_head">
          {/* <h2>Compact for rent in Abu Dhabi & Dubai</h2> */}
          <h2>Compact and Efficient: Compact Rentals in Abu Dhabi & Dubai</h2>
        </div>
        <div className="car_off_text">
          <p>
            We feature a broad range of trendy Compact car rental deals in Abu
            Dhabi & Dubai.
          </p>
        </div>
        {!loader ? (
