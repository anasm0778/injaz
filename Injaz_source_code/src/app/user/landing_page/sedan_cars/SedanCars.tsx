...
const SedanCars: React.FC<SedanCars> = ({ data, phoneData }) => {
  const [Rows, setrows] = useState([]);
  const [loader, setLoader] = useState(true);
  const router = useRouter();
  useEffect(() => {
    if (data.length > 0) {
      const newData = data?.filter((item: any) => item.category === "SEDAN");
      setrows(newData);
      setLoader(false);
    }
  }, [data]);
    ...
    <div className="car_offers">
      <Container maxWidth="xl">
        <div className="car_off_head">
          {/* <h2>Sedan for rent in Abu Dhabi & Dubai</h2> */}
          <h2>Cruise in Comfort: Sedan Rentals in Abu Dhabi & Dubai</h2>
        </div>
        <div className="car_off_text">
          <p>
            Explore Chic Sedan Rentals in Abu Dhabi & Dubai: Trendy Deals Await
            Your Journey in the Emirates!
          </p>
        </div>
        {!loader ? (
          <Carousel
