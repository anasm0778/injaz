...
interface ExoticCars {
  data: any;
  phoneData: any;
}

const ExoticCars: React.FC<ExoticCars> = ({ data, phoneData }) => {
  const [Rows, setrows] = useState([]);
  const [loader, setLoader] = useState(true);
  const router = useRouter();
  useEffect(() => {
    if (data.length > 0) {
      const newData = data?.filter((item: any) => item.category === "LUXURY");
      setrows(newData);
      setLoader(false);
    }
  }, [data]);
    ...
    <div className="car_offers">
      <Container maxWidth="xl">
        <div className="car_off_head">
          {/* <h2>Luxury for rent in Abu Dhabi & Dubai</h2> */}
          <h2>
            Experience Elegance: Luxury Car Rentals Available in Abu Dhabi &
            Dubai
          </h2>
        </div>
        <div className="car_off_text">
          <p>
            We feature a broad range of trendy Luxury car rental deals in Abu
            Dhabi & Dubai.
          </p>
        </div>
        {!loader ? (
