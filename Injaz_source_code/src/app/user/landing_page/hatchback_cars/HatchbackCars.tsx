...
interface HatchbackCars {
  data: any;
  phoneData: any;
}

const HatchbackCars: React.FC<HatchbackCars> = ({ data, phoneData }) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      const newData = data?.filter(
        (item: any) => item.category === "HATCHBACK"
      );
      setrows(newData);
      setLoader(false);
    }
    ...
    <div className="car_offers">
      <Container maxWidth="xl">
        <div className="car_off_head">
          {/* <h2>Hatchback for rent in Abu Dhabi & Dubai</h2> */}
          <h2>Hatchback and Nimble: Hatchback Rentals in Abu Dhabi & Dubai</h2>
        </div>
        <div className="car_off_text">
          <p>
            We feature a broad range of trendy Hatchback car rental deals in Abu
            Dhabi & Dubai.
          </p>
        </div>
        {!loader ? (
